...
export class ListProduitsComponent {
  ...
  ajusterTVA(produit: Produit) {
    produit.tauxTVA = 0.20;
  }
  isLowPriced(product: Produit): boolean {
    return product.prixUnitaire <= 50;
  }
  searchCode: string = '';
  ...
  searchProduct() {
    if (this.searchCode === '') {
      ...
      this.filteredProduits = this.listProduits.filter(produit =>
        produit.code.toLowerCase().startsWith(this.searchCode.toLowerCase())
      );
    }
  }
  addProduit() {
    this.listProduits.push(this.newProduit); 
